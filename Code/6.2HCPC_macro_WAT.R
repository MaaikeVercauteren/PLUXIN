...
data_full_WAT_macro_spot<-as.data.frame(read_csv("Final analysis/Final dataset/Datasplits/data_full_WAT_macro_spot.csv"))
...
col_to_keep <- setdiff(colnames(WAT_macro_cluster_sample_2km),"Unique.Sample.Identifier")
rows_to_modify <- apply(WAT_macro_cluster_sample_2km[col_to_keep], 1, function(x) any(x == -Inf))

# Set all values in the specified row to 0, except for the chosen column
WAT_macro_cluster_sample_2km[rows_to_modify, col_to_keep] <- 0

length(unique(WAT_macro_cluster_sample_2km$ `Unique.Sample.Identifier`))
#72 samples
...
res<-PCA(WAT_macro_cluster_sample_2km,ncp=Inf,graph=FALSE)
res$eig #3 dimensions explain 95.74% of variance 


#nieuwe analyse met bepaald aantal dimensies
res.pca.macro.wat<-PCA(WAT_macro_cluster_sample_2km,ncp=3,graph=FALSE)
...
write.csv(result_HCPC_macro_WAT, "Final analysis/Results/result_HCPC_macro_WAT.csv")
